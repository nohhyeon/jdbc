...
import su.dept.service.DeptService;

public class DeptDAO implements DeptService {
	private static Log log = LogFactory.getLog(DeptDAO.class); // 제네릭 DeptDTO 클래스 반환 타입으로 전체 데이터를 조회한다.

	// 제네릭 DeptDTO 클래스 반환 타입으로 전체 데이터를 조회한다.
	@Override
	...
	public DeptDTO deptSelect(int deptno) {
		...
		try {
			...
			// SQL인 select...from...where 문을 실행하고 데이터를 조회한다.
			resultSet = preparedStatement.executeQuery(); // 커서를 현재 위치에서 한 행 앞으로 이동하면서 반복한다.
			while (resultSet.next()) {
